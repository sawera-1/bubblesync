...
const ViewNoteScreen = ({ route, navigation }) => {
        ...
        (doc) => {
          if (doc.exists) {
            ...
            setNote({
              id: doc.id,
              ...data,
              text: data.body || data.convertedTranscript || data.detail || 'No detailed content.',
  ...
  const handleEdit = () => note?.id && navigation.navigate('ViewEditNote', { noteId: note.id });
  ...
  const isStarred = note.isStarred === true;
    ...
    <SafeAreaView style={[styles.safeArea, { paddingTop: androidStatusPadding }]}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.descriptionText}>{note.text}</Text>
        </View>
        ...
        <View style={styles.actionButtonContainer}>
          <TouchableOpacity style={[styles.actionButton, styles.editButton]} onPress={handleEdit}>
            <MaterialCommunityIcons name="pencil-outline" size={24} color={COLORS.PrimaryAccent} style={{ marginRight: 8 }} />
            <Text style={[styles.actionButtonText, { color: COLORS.PrimaryAccent }]}>Edit Note</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.actionButton, styles.deleteButton, { backgroundColor: COLORS.SecondaryAccent }]} onPress={handleDelete}>
            <MaterialCommunityIcons name="delete-outline" size={24} color={COLORS.White} style={{ marginRight: 8 }} />
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: COLORS.White, borderRadius: 16, padding: 15, marginBottom: 15, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 3 },
  ...
  statusCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 15, marginBottom: 20, backgroundColor: COLORS.White, borderRadius: 16, borderWidth: 2 },
  statusText: { marginLeft: 10, fontSize: 18, fontWeight: '900', color: COLORS.MainText, flexShrink: 1 },

  infoRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 },
  infoItem: { backgroundColor: COLORS.White, borderRadius: 16, padding: 15, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 3 },
  ...
  deleteButton: { shadowColor: COLORS.SecondaryAccent },
  actionButtonText: { color: COLORS.White, fontSize: 16, fontWeight: '900' },

  metadataContainer: { flexDirection: 'row', alignItems: 'center', marginTop: 5 },
  metadataText: { fontSize: 14, color: COLORS.PrimaryAccent, marginLeft: 10, fontWeight: '600' },
});
